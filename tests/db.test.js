...
test('a fresh database creates the full schema and records the migration', () => {
  ...
  assert.deepEqual(applied.map((m) => m.id).sort(), [1, 2, 3], 'all migrations must be recorded');
  ...
  assert.deepEqual(applied.map((m) => m.name), ['init', 'live_orders_client_id_unique', 'live_order_events_append_only']);
...
test('migrations are idempotent', () => {
  ...
  assert.equal(appliedMigrations(db).length, 3);
  db.close();
  fs.rmSync(dir, { recursive: true, force: true });
});
